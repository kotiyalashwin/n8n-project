...
export default function NodeForm({
  ...
  const selectedMode = useState("send");
  const [selectedTools, setSelectedTools] = useState<string[]>([]);
  const [selectedCredentials, setSelectedCredentials] = useState("");
  const availableTools = ["telegram", "gmail"];
  const [dymanicCredentialFiels, setDynamicCredentialFields] = useState<
    { label: string; name: string; type: string; required: boolean }[]
  >([]);
  const handleToolSelection = (tool: string) => {
    const reqCred = nodeConfigs[tool as keyof typeof nodeConfigs].credentials;
    setDynamicCredentialFields((prev) => {
      return [...prev, ...reqCred];
    });
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    const finalData = [
      ...
      {
        name: "producesOutput",
        value: config.modes?.find((m) => m.value === selectedMode)
          ?.producesOutput
          ? "true"
          : "false",
      },
      ...(selectedMode
        ? [{ name: "mode", value: selectedMode as string }]
        : []),
    ];
  ...
  return (
    <div className="h-full">
      <form onSubmit={handleSubmit} className="flex h-full flex-col gap-4">
        ...
        {type === "ai" && (
          <div>
            <label className="text-foreground mb-2 block">Tools:</label>
