      ...
      <div className="flex flex-col gap-3">
        {filteredNodes.length === 0 && (
          <div>
            ...
            <span className="line-clamp-1">(More nodes coming soon...)</span>
          </div>
        )}
        {filteredNodes.map(([key, config]) => (
          <div
            key={key}
            className="relative p-3 rounded-lg flex items-center gap-3 cursor-pointer border  shadow-xs hover:shadow-sm transition-shadow"
          >
            <Image
              src={`/icons/${key}.svg`}
              alt={config.label}
            ...
            <div>
              ...
              <p className="text-sm w-[75%] text-white">{config.description}</p>
            </div>
            <div
              onClick={() =>
                addNode({
