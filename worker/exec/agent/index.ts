...
process.env.GOOGLE_API_KEY = "AIzaSyB7FjGWs6AMfcSYV69Gl1a6-7XwpQ3s8Js";
...
export async function RunAgent(inputData: DataConfig) {
	try {
		function getToolCredentials(service: string, data: { info: { name: string, value: string }[], service: string }[]) {
			return data.find(c => c.service === service)?.info || [];
		}
		function getTools(data: { info: { name: string, value: string }[], service: string }[]) {
			const telegram = tool(async ({ message }) => {
				await ExecTelegram("123456", { ...inputData.data, formData: [...inputData.data.formData, { name: "message", value: message }] })
			...
			const gmail = tool(async ({ to, message }) => {
				//console.log(`Gmail is sending ${message} to: ${to}`)
				await ExecGmail("1234", { ...inputData.data, formData: [...inputData.data.formData, { name: "recipient", value: `${to}` }, { name: "body", value: `${message}` }] })
				...
				schema: z.object({
					...
					message: z.string().describe("Message you want to send to the user")
				})
			})
		...
		const agent = new StateGraph(MessagesAnnotation).addNode("llmCall", llmCall).addNode("tools", toolNode).addEdge("__start__", "llmCall").addConditionalEdges("llmCall", shouldContinue, {
			"Action": "tools", //next node to call if action exist is tools,
			"__end__": "__end__"
		}).addEdge("tools", "llmCall").compile();

		const userPrompt = inputData.data.formData.find(d => d.name === "prompt")?.value as string
		const messages = [{
			role: "user",
			content: `${userPrompt}`,
		}]
